...
class DataFoursquare(object):
    ...
    def save_variables(self):
        ...
        with open(self.SAVE_PATH + self.save_name + '.json', 'w') as outfile:
            ujson.dump(foursquare_dataset, outfile)

    def load_json(self):
        # data = ujson.loads(open(self.data_path + self.data_name + '.json', 'rb'))
        json_file = open('/home/local/ASUAD/ychen404/Code/DeepMove_new/data/tweets-cikm.json')
        
        with open(self.SAVE_PATH + self.save_name + '.json') as json_file:
            data_json = ujson.load(json_file)
        return data_json
...
if __name__ == '__main__':
    ...
    print('Loading json from {}'.format(data_generator.SAVE_PATH + data_generator.save_name + '.json'))
    data_json = data_generator.load_json()
    
    print(data_json.keys())
    # print(data_json['data_neural'].keys())
    # pdb.set_trace()
    x = data_json['data_neural'].keys()
    print(x)
    # y = x.replace('\'', "")
    # print(y)